...
from src.tokenizer_modules.tokenizer_utils import load_bpe_file
...
class Llama3Tokenizer:
    ...
    def get_instance(cls, model_path: Path):
        ...
        if _INSTANCE is None:
            _INSTANCE = Llama3Tokenizer(model_path=model_path)
        return _INSTANCE
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(vocab_size={self.n_words}, backend={type(self.model).__name__})"
...
if __name__ == "__main__":
    # Usage
    tokenizer = Llama3Tokenizer.get_instance(model_path=Path(__file__).parent.parent / "tokenizer-models" / "llama3" / "llama3_tokenizer.model")
    tokens = tokenizer.encode("ఎలా టైపు చెయ్యాలో వివరంగా తెలుసుకోండి Hello, how are you?", bos=False, eos=False)
    for token in tokens:
